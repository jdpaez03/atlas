...
**On-demand digest**: `POST /digests/run {days: 1..14 = 1}` returns the `Mission` "CC digest · last N day(s)" and runs it in the background. It uses the scan lock and the same 409/422 rules as `/inbox/scan`. It lists the messages of the last N days, **ignoring processed ids and `last_scan`**, so mail that an earlier scan already processed can still be digested. It runs only the digest step (the same selection, rules and summaries, with `email_read` evidence per body read); there is no extraction and no staleness pass. `asks_me` still creates or updates follow-ups through the normal de-duplication. It never changes `state.json`. Its report reads "CC digest · last N days: C CC candidate(s) · S skipped · T thread(s)".
**Empty result**: when a scan or a run has no candidates, the reason is logged to the activity feed ("CC digest · No CC emails in the last N days" / "… among the N new emails", or "CC digest · N CC emails, all excluded by rules/automated", or a note that ATLAS doesn't know the user's addresses) and added to the report (a scan adds "CC digest: none · <reason>"). A scan with no new messages runs no digest step at all.
